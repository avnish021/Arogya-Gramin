...
{	 
    ...
    $rdt = $_POST['rdt'];
    try {
	 $sql = "INSERT INTO `cpr` (`qunt`, `rate`, `price`, `discount`, `placed`, `rdt`, `status`, `user_type`) VALUES ('$qunt', '$rate', '$price', '$discount', '$placed', '$rdt', 'True', 'Volunteer')";
  
	 if (mysqli_query($conn, $sql)) {
         echo "<script>alert('Your CPR Transaction Upload successfully !..');window.location.href='cpr.php'</script>";
		//header('Location: ');
	 }
  } 
  catch(Exception $e) {
        ...
        <div class="row">
          <div class="col-sm-6">
            <label>Quantity</label>
              ...
              <input id="uname" name="qunt" type="text" class="form-control" placeholder="Your answer">
           <br>
          </div>
          <!-- /.col-lg-6 -->
          <div class="col-sm-6">
            <label>Rate/unit</label>
              ...
              <input id="uemail" name="rate" type="text" class="form-control" placeholder="Your answer">
          <br>
          </div>
          <div class="col-sm-6">
            <label>Pricing</label>
          ...
          <div class="col-sm-6">
           ...
           <textarea name="placed" class="form-control"></textarea>
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row['id']."</td><td>".$row['qunt']."</td><td>".$row['rate']."</td><td>Rs".$row['price']."</td><td>".$row['discount']."%</td><td>".$row['placed']."</td><td>".$row['status']."</td><td>".$row['rdt']."</td><td><center><a href='edit_cpr.php?id=".$row['id']."' class='btn btn-info'><i class='fa fa-edit'></i></a></center></td></tr>";
    }
} else {
    echo "0 results";
}
