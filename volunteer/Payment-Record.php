...
if (isset($_POST)) {
    ...
    if ($name != "" && !$email = "" && $quantity != "" && $cardamt != "" && $discount != "" && $tot_amount != "" && $status
        ...
        $arr = array(
            "TYPE" => $type, "TRANS_ID" => $trnsid, "V_NAME" => $name, "V_EMAIL" => $_POST['UserMail'], "QUANTITY" => $quantity, "AMOUNT_EVERY_CARD" => $cardamt, "DISCOUNT" => $discount, "TOTAL_AMOUNT" => $tot_amount, "PAYMENT_STATUS" => $status, "ERROR_CODE" => "", "ERROR_DESCRIPTION" => "",
            "ERROR_SOURCE" => "", "ERROR_STEP" => "", "ERROR_REASON" => "", "PAYMENT_ID" => "", "CONTACT" => $contact, "MODE" => $mode, "DATE" => $date
        );
